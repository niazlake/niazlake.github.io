...
//удален IMG, берется как бэкграунд из CSS
 ...
 **********************************************************************************************/
	...
	this.renderBasketTable = function()
		...
		if ($('#bcontainer').length == 0)
			{//удален IMG, берется как бэкграунд из CSS
				//вместо button class='bbuton' теперь <span>
				// вызов скрпта модального окна перенесено на общий div bfooter
			$("body").append(" \
				<div id='blindLayer' class='blindLayer'></div> \
				<div id='bcontainer' class='bcontainer'> \
				<div id='top-pic'/> <div id='corner-left-pic'/> <div id='corner-right-pic'/>\
				<a id='bclose-window' href='#' onclick='" + this.objNAME + ".closeWindow(\"bcontainer\", 1);'></a> \
				...
				<div id='bfooter' onclick=\"cart.showWinow('order', 1)\"  data-dismiss = 'modal'> <span class='bbutton' >" + local.order + "</span><span id='bsum'>...</span></div> \
				</div> \
			");
		...
		for(var idkey in this.DATA)
			...
			with (this.DATA[idkey])
				...
				var productLine = '<tr class="bitem" id="wigoodline-' + id + '"> \
													...
													<td><a class="bclose-item" href="#" onclick="' + this.objNAME + '.delItem(\'' + id + '\')"></a></td> \
													</tr>';
				}
